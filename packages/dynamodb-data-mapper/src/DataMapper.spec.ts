...
import type { Schema } from "@invisit/dynamodb-data-marshaller"
import {
  AttributePath,
  between,
  equals,
...
import type { BatchGetOptions, ParallelScanState, GlobalSecondaryIndexOptions } from "./index"
import type {
  BatchGetItemInput,
  BatchWriteItemInput,
  DescribeTableOutput,
...
import { ProvisionedCreateTableOptions } from "./index"

type BinaryValue = ArrayBuffer | ArrayBufferView

const defaultProvisionedOptions: ProvisionedCreateTableOptions = {
  readCapacityUnits: 5,
...
describe("DataMapper", () => {
    ...
    for (const asyncInput of [true, false]) {
      it("should should partition get batches into requests with 100 or fewer items", async () => {
        ...
        for (let i = 0; i < 325; i++) {
          ...
          expected[Math.floor(i / 100)][0].RequestItems.foo.Keys.push({ fizz: { N: String(i) } })
        }
      ...
      it("should should retry unprocessed items", async () => {
        ...
        for (let i = 0; i < 325; i++) {
          gets.push(new Item(i))
          expected[currentRequest][0].RequestItems.foo.Keys.push({ fizz: { N: String(i) } })
          ...
          if (failures.has(String(i))) {
            responses[currentRequest].UnprocessedKeys.foo.Keys.push({ fizz: { N: String(i) } })
  ...
  describe("#createGlobalSecondaryIndex", () => {
    ...
    it("should make and send an UpdateTable request", async () => {
      ...
      expect(mockDynamoDbClient.waitFor.mock.calls).toEqual([["tableExists", { TableName: "foo" }]])
    })
  })
  ...
  describe("#createTable", () => {
    ...
    it("should make and send a CreateTable request", async () => {
      await mapper.createTable(Item, defaultProvisionedOptions)
    ...
    it("should forgo invoking the waiter if the table is already active", async () => {
      createTablePromiseFunc.mockImplementationOnce(() =>
        Promise.resolve({
    ...
    describe("index keys", () => {
      ...
      it("should throw if no options were provided for a modeled index", async () => {
        const options: ProvisionedCreateTableOptions = {
          readCapacityUnits: 5,
          writeCapacityUnits: 5,
          billingMode: "PROVISIONED"
        }
  ...
  describe("#delete", () => {
    ...
    it("should not include a condition expression when the schema contains a version attribute but the value is undefined", async () => {
      ...
      expect((mockDynamoDbClient.deleteItem.mock.calls[0] as any)[0]).not.toHaveProperty("ConditionExpression")
    })
    ...
    it("should not include ExpressionAttributeValues when a substitution has not been made", async () => {
      ...
      expect((mockDynamoDbClient.deleteItem.mock.calls[0] as any)[0]).toEqual({
    ...
    it("should return instances of the correct class", async () => {
      ...
      const result = await mapper.delete(new Item("buzz"), { returnValues: "ALL_OLD" })
  ...
  describe("#deleteTable", () => {
    ...
    it("should make and send a DeleteTable request and wait for it to take effect", async () => {
      ...
      expect(mockDynamoDbClient.deleteTable.mock.calls).toEqual([[{ TableName: "foo" }]])

      expect(mockDynamoDbClient.waitFor.mock.calls).toEqual([["tableNotExists", { TableName: "foo" }]])
    })
  })

  describe("#ensureGlobalSecondaryIndexExists", () => {
    const waitPromiseFunc = jest.fn(() => Promise.resolve())
    const describeTablePromiseFunc = jest.fn(() =>
      Promise.resolve({
        Table: {
          TableStatus: "ACTIVE",
          GlobalSecondaryIndexes: [
    ...
    it("should resolve immediately if the table exists, is active, and the GSI already exists", async () => {
      ...
      expect(mockDynamoDbClient.describeTable.mock.calls).toEqual([[{ TableName: tableName }]])
  ...
  describe("#ensureTableExists", () => {
    ...
    it("should resolve immediately if the table exists and is active", async () => {
      await mapper.ensureTableExists(Item, defaultProvisionedOptions)
    ...
    it('should attempt to create the table if "describeTable" throws a "ResourceNotFoundException"', async () => {
      ...
      const options = defaultProvisionedOptions
    ...
    it('should rethrow any service exception other than "ResourceNotFoundException"', async () => {
      describeTablePromiseFunc.mockImplementationOnce(() => Promise.reject(new Error("PANIC")))

      const options = defaultProvisionedOptions

      await expect(mapper.ensureTableExists(Item, options)).rejects.toMatchObject(new Error("PANIC"))
  ...
  describe("#ensureTableNotExists", () => {
    ...
    it('should wait for the table not to exist if its state is not "DELETING"', async () => {
      ...
      expect(mockDynamoDbClient.waitFor.mock.calls).toEqual([["tableNotExists", { TableName: tableName }]])
      expect((mapper.deleteTable as any).mock.calls.length).toBe(0)
    })
    ...
    it('should wait for the table to exist if its state is "CREATING", then delete it', async () => {
      ...
      expect(mockDynamoDbClient.waitFor.mock.calls).toEqual([["tableExists", { TableName: tableName }]])
      expect((mapper.deleteTable as any).mock.calls.length).toBe(1)
    })
  ...
  describe("#get", () => {
    ...
    it("should convert an empty (item not found) response into a rejected promise whose rejection includes the request sent to DynamoDB", () => {
            ...
            Items: [
              {
                ...
                baz: { L: [{ BOOL: index % 2 === 0 }, { N: (++index).toString(10) }] }
              }
            ],
  ...
  describe("#put", () => {
    ...
    it("should include a condition expression requiring that no versioned item be present when the schema contains a version attribute but the value is undefined", async () => {
      ...
      expect((mockDynamoDbClient.putItem.mock.calls[0] as any)[0]).toEqual({
        Item: {
          foo: { S: "buzz" },
          pop: { N: "0" }
        },
    ...
    it("should not include a condition expression when the skipVersionCheck input parameter is true", async () => {
      ...
      expect((mockDynamoDbClient.putItem.mock.calls[0] as any)[0]).not.toHaveProperty("ConditionExpression")
    })
  ...
  describe("#query", () => {
    ...
    it("should request a consistent read if the readConsistency is StronglyConsistent", async () => {
      ...
      expect(mockDynamoDbClient.query.mock.calls[0][0]).toMatchObject({ ConsistentRead: true })
    })
    ...
    it("should allow a condition expression as the keyCondition", async () => {
      ...
      expect(mockDynamoDbClient.query.mock.calls[0][0]).toMatchObject({
    ...
    it("should allow a projection expression", async () => {
      const results = mapper.query(QueryableItem, { snap: "crackle" }, { projection: ["snap", "fizz[1]"] })
    ...
    it("supports the legacy call pattern", async () => {
      ...
      expect(mockDynamoDbClient.query.mock.calls[0][0]).toEqual({
        TableName: "foo",
    ...
    describe("startKey serialization", () => {
      ...
      it("should not inject default values into the startKey", async () => {
        ...
        expect(mockDynamoDbClient.query.mock.calls[0][0].ExclusiveStartKey).toEqual({
          snap: { S: "key" }
        })
      })
    })
  ...
  describe("#scan", () => {
    ...
    it("should request a consistent read if the readConsistency is StronglyConsistent", async () => {
      const results = mapper.scan(ScannableItem, { readConsistency: "strong" })

      await results.next()

      expect(mockDynamoDbClient.scan.mock.calls[0][0]).toMatchObject({ ConsistentRead: true })
    })
    ...
    it("should allow a filter expression", async () => {
      ...
      expect(mockDynamoDbClient.scan.mock.calls[0][0]).toMatchObject({
    ...
    it("should allow a projection expression", async () => {
      const results = mapper.scan(ScannableItem, { projection: ["snap", "fizz[1]"] })
    ...
    it("should allow the page size to be set", async () => {
      ...
      expect(mockDynamoDbClient.scan.mock.calls[0][0]).toMatchObject({ Limit: 20 })
    })
    ...
    describe("startKey serialization", () => {
      ...
      it("should not inject default properties into the startKey", async () => {
        ...
        expect(mockDynamoDbClient.scan.mock.calls[0][0].ExclusiveStartKey).toEqual({
          snap: { S: "key" }
        })
      })
    })
  ...
  describe("updating items", () => {
    ...
    describe("#update", () => {
      ...
      it("should use the table name specified in the supplied table definition", async () => {
        ...
        expect(mockDynamoDbClient.updateItem.mock.calls[0][0]).toMatchObject({ TableName: tableName })
      })
      ...
      it("should apply a table name prefix provided to the mapper constructor", async () => {
        ...
        expect(mockDynamoDbClient.updateItem.mock.calls[0][0]).toMatchObject({ TableName: tableNamePrefix + tableName })
      })
      ...
      it("should marshall updates into an UpdateItemInput", async () => {
        ...
        expect(mockDynamoDbClient.updateItem.mock.calls[0][0]).toMatchObject({
          TableName: tableName,
          Key: {
            fizz: { S: "key" }
          },
          ...
          ExpressionAttributeValues: {
            ":val1": {
              L: [{ N: "1" }, { B: Uint8Array.from([0xde, 0xad, 0xbe, 0xef]) }]
            }
          },
      ...
      it("should throw an error if no attributes were returned", async () => {
        ...
        return expect(
          ...
          new Error("Update operation completed successfully, but the updated value was not returned")
        )
      })
      ...
      describe("version attributes", () => {
        ...
        it("should not include a condition expression when the skipVersionCheck input parameter is true", async () => {
          ...
          expect(mockDynamoDbClient.updateItem.mock.calls[0][0]).not.toHaveProperty("ConditionExpression")
        })
