...
import { Schema, unmarshallItem, ZeroArgumentsConstructor } from "@invisit/dynamodb-data-marshaller"
...
export abstract class Paginator<T> implements AsyncIterableIterator<Array<T>> {
  ...
  protected lastKey?: T
  protected lastResolved: Promise<IteratorResult<Array<T>>> = Promise.resolve() as any

  protected constructor(
    protected readonly paginator: DynamoDbPaginatorInterface,
    protected readonly valueConstructor: ZeroArgumentsConstructor<T>
  ) {
  ...
  protected async getNext(): Promise<IteratorResult<Array<T>>> {
    return this.paginator.next().then(({ value = {}, done }) => {
      if (!done) {
        ...
        return {
          value: (value.Items || []).map((item: any) => unmarshallItem(this.itemSchema, item, this.valueConstructor)),
          done: false
        }
      }
