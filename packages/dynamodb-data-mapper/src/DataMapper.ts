...
export class DataMapper {
  ...
  async *batchWrite<T extends StringToAnyObjectMap>(
    ...
    for await (const [tableName, { DeleteRequest, PutRequest }] of batch) {
      const { keyProperties, itemSchemata } = state[tableName]
      const attributes = PutRequest ? PutRequest.Item : (DeleteRequest || { Key: {} }).Key
      const { constructor, schema } = itemSchemata[itemIdentifier(attributes, keyProperties)]

      yield [PutRequest ? "put" : "delete", unmarshallItem<T>(schema, attributes, constructor)]
    }
  }
  ...
  async createTable(valueConstructor: ZeroArgumentsConstructor<any>, options: CreateTableOptions) {
    const def = this.getTableResourceDef(valueConstructor, options)
    const { TableName } = def

    const { TableDescription: { TableStatus } = { TableStatus: "CREATING" } } = await this.client
      .createTable(def)
      .promise()
  ...
  async createGlobalSecondaryIndex(
    ...
    const { TableDescription: { TableStatus } = { TableStatus: "UPDATING" } } = await this.client
      .updateTable({
  ...
  async ensureGlobalSecondaryIndexExists(
    ...
    const TableName = this.getTableName(valueConstructor.prototype)
    try {
      const { Table: { GlobalSecondaryIndexes } = { GlobalSecondaryIndexes: [] } } = await this.client
  ...
  async ensureTableNotExists(valueConstructor: ZeroArgumentsConstructor<any>) {
    const TableName = this.getTableName(valueConstructor.prototype)
    try {
      const { Table: { TableStatus: status } = { TableStatus: "CREATING" } } = await this.client
        .describeTable({ TableName })
        .promise()
  ...
  async put<T extends StringToAnyObjectMap = StringToAnyObjectMap, Ctor extends ZeroArgumentsConstructor<T> = ZeroArgumentsConstructor<T>>(
    ...
    return unmarshallItem<T>(schema, req.Item, item.constructor as ZeroArgumentsConstructor<T>)
  }
  ...
  scan<T extends StringToAnyObjectMap>(
    ctorOrParams: ZeroArgumentsConstructor<T> | ScanParameters<T> | ParallelScanWorkerParameters<T>,
    options: ScanOptions | ParallelScanWorkerOptions = {}
  ): ScanIterator<T> {
    let valueConstructor: ZeroArgumentsConstructor<T>
    if (
  ...
  private async *mapWriteBatch<T extends StringToAnyObjectMap>(
    ...
    for await (const [type, item] of items) {
      ...
      const marshalled =
        type === "delete" ? { DeleteRequest: { Key: attributes } } : { PutRequest: { Item: attributes } }
...
function attributeDefinitionList(attributes: AttributeTypeMap): Array<AttributeDefinition> {
...
function convertBatchGetOptions(options: BatchGetTableOptions, itemSchema: Schema): TableOptions {
  ...
  if (options.projection) {
    ...
    out.ProjectionExpression = serializeProjectionExpression(
      options.projection.map(propName => toSchemaName(propName, options.projectionSchema || itemSchema)),
      attributes
    )
...
function itemIdentifier(marshalled: AttributeMap, keyProperties: Array<string>): string {
