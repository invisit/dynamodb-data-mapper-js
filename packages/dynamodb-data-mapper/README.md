...
DynamoDB. Powered by the `@invisit/dynamodb-data-marshaller` and
`@invisit/dynamodb-expressions` packages, using `DataMapper` lets you define each
...
```typescript
import {DynamoDbSchema, DynamoDbTable} from '@invisit/dynamodb-data-mapper';
...
* (Optional) An object specifying any of the following options:
    ...
    * `perTableOptions` - An object whose keys are table names and whose values
        ...
        * `projection` - A projection expression directing DynamoDB to return a
            ...
            documentation for the `@invisit/dynamodb-expressions` package for
...
* (Optional) An object specifying any of the following options:
    ...
    * `condition` - A condition expression whose assertion must be satisfied in
        ...
        documentation for the `@invisit/dynamodb-expressions` package for guidance
...
* The condition that specifies the key value(s) for items to be retrieved by the
    ...
    Please refer to the documentation for the `@invisit/dynamodb-expressions`
...
* The expression to execute. Please refer to the documentation for the
    `@invisit/dynamodb-expressions` package for guidance on creating update
    expression objects.
